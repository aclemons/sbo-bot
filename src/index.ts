...
export = (app: Probot) => {
  app.on('issue_comment.created', async (context) => {
    ...
    if (build_arch) {
      ...
      const { data, status } = await axios.post(process.env.JENKINS_WEBHOOK, { build_arch: build_arch, 'gh_pr': payload.issue.number, build_package: build_package }, { headers: { 'Content-Type': 'application/json', token: process.env.JENKINS_WEBHOOK_SECRET } })

      context.log.info(`Received jenkins response: ${JSON.stringify(data)}`);

      if (status === 200 && data['triggered']) {
      ...
      let { data, status } = await axios.post(process.env.JENKINS_WEBHOOK, { build_arch: 'i586', 'gh_pr': payload.issue.number, build_package: build_package }, { headers: { 'Content-Type': 'application/json', token: process.env.JENKINS_WEBHOOK_SECRET } })
      ...
      if (status === 200 && data["jobs"]["slackbuilds.org-pr-check-build-package"]['triggered']) {
        ...
        let { data, status } = await axios.post(process.env.JENKINS_WEBHOOK, { build_arch: 'amd64', 'gh_pr': payload.issue.number, build_package: build_package }, { headers: { 'Content-Type': 'application/json', token: process.env.JENKINS_WEBHOOK_SECRET } })
